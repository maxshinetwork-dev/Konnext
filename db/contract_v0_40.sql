...
-- #####################################################################
-- ##  v0.40：把 v0.39 那道授权门的【漏网之鱼】补上
-- ##
-- ##  ★用户 2026-08-10 当天追问：「如果没有休假，这个对高级管理人员的限制
-- ##    应该对所有部门的任何更改都生效。你自己查查是不是有遗漏。」
-- ##  —— 查了，确实有，而且不止一处。全库扫出来四张表的写口绕过了那道门。
-- ##
-- ##  ★根子上的错：v0.39 的 fn_admin_may_write 把「没登记部门归属」当成
-- ##    「这是决策级本职 → 放行」。方向反了。
-- ##    没登记 ≠ 决策级，绝大多数时候是【忘了登记】。
-- ##    于是每一张忘了登记的业务表，对管理员都是敞开的 —— 而且不报错。
-- ##  改成：★只有【明确登记成空数组】才算决策级本职；没登记一律【拒绝】。
-- ##    宁可当场报错让人去登记，也不要静默放行。
-- #####################################################################

-- ── ① 十张决策级表：明确登记为「无部门归属」──────────────────────
--    这些本来就是决策管理员的活，跟部门授权无关。
--    ★现在必须【明确写下来】，不能再靠「查不到就当是本职」蒙混过去。
INSERT INTO table_ownership(table_name, write_dept, note) VALUES
 -- ★eng_setting 的写权限是【按键】控制的（见 eng_setting_upd 策略），表级归属不适用；
 --   登记成空数组只是表示「不按表判」，绝不是说管理员能随便改所有键
 ('eng_setting',           ARRAY[]::text[], 'v0.40 ★按【键】控制（write_depts 列），表级归属不适用'),
 ('app_account',           ARRAY[]::text[], 'v0.40 决策级：账号是决策页的活，跟部门授权无关'),
 ('account_department',    ARRAY[]::text[], 'v0.40 决策级：改部门归属＝提权，只有决策管理员'),
 ('account_reset_log',     ARRAY[]::text[], 'v0.40 决策级：重置留痕，只增不改'),
 ('assertion_def',         ARRAY[]::text[], 'v0.40 决策级：能删这里就能把系统自检悄悄关掉'),
 ('money_visibility',      ARRAY[]::text[], 'v0.40 决策级：金额可见性口径'),
 ('project_scope_registry',ARRAY[]::text[], 'v0.40 决策级：项目归属登记'),
 ('table_ownership',       ARRAY[]::text[], 'v0.40 决策级：★这张表自己也要登记，否则改它就能绕开整道门'),
 ('status_stall_threshold',ARRAY[]::text[], 'v0.40 决策级：停留阈值'),
 ('dept_handoff_rule',     ARRAY[]::text[], 'v0.40 决策级：跨部门交接规则'),
 ('audit_log',             ARRAY[]::text[], 'v0.40 决策级：全公司留痕，只增不改'),
 ('handoff_log',           ARRAY[]::text[], 'v0.40 决策级：交接留痕，只增不改')
ON CONFLICT (table_name) DO UPDATE SET write_dept = EXCLUDED.write_dept, note = EXCLUDED.note;
-- dept_delegation 在 v0.39 已登记成空数组（授权表挂部门＝自己给自己授权的循环）

-- ── ② 三张【漏网的业务表】：登记归属 + 走同一道门 ────────────────
INSERT INTO table_ownership(table_name, write_dept, note) VALUES
 ('work_log',     ARRAY['eng_mgmt','maintenance'],
   'v0.40 ★补登记：打卡是工程/运维的业务数据，之前没登记 → 管理员改它完全不受授权约束'),
 ('daily_report', ARRAY['eng_mgmt'],
   'v0.40 ★补登记：每日上报（含补录）是工程线的活'),
 ('daily_issue',  ARRAY['eng_mgmt'],
   'v0.40 ★补登记：问题上报随每日上报走')
ON CONFLICT (table_name) DO UPDATE SET write_dept = EXCLUDED.write_dept, note = EXCLUDED.note;

INSERT INTO project_scope_registry(table_name, kind, kind_cn, trace_path, note) VALUES
 ('work_log','project_required','项目必填','project_id','打卡一定在某个项目现场')
ON CONFLICT (table_name) DO NOTHING;

-- ★方向掉过来：没登记 = 拒绝，不是放行
CREATE OR REPLACE FUNCTION fn_admin_may_write(p_table text) RETURNS boolean AS $$
    SELECT CASE
      -- ① 明确登记成【空数组】＝ 决策级本职（账号 / 断言 / 登记表 / 留痕…）→ 放行
      WHEN EXISTS(SELECT 1 FROM table_ownership o
                   WHERE o.table_name = p_table
                     AND coalesce(array_length(o.write_dept,1),0) = 0) THEN true
      -- ② 有部门归属 → 该表所属的【任一】部门授权了才行
      WHEN EXISTS(SELECT 1 FROM table_ownership o, unnest(o.write_dept) AS d
                   WHERE o.table_name = p_table AND fn_dept_delegated(d)) THEN true
      -- ③ ★压根没登记 → 拒绝。没登记不等于决策级，绝大多数时候是忘了登记；
      --    当成本职放行的话，每一张忘登记的业务表对管理员都是敞开的，而且不报错
      ELSE false END;
$$ LANGUAGE sql STABLE SECURITY DEFINER;
COMMENT ON FUNCTION fn_admin_may_write IS
  '★最高管理者默认对别的部门只读。v0.40 把默认方向掉过来了：'
  '登记成空数组＝决策级本职（放行）· 有部门归属＝要授权 · ★没登记＝拒绝。'
  '「查不到就放行」是 v0.39 的洞 —— 忘了登记的表会静默敞开';

-- 三张业务表的写策略改走同一道门（保留施工人员写自己那行的分支）
DROP POLICY IF EXISTS work_log_ins ON work_log;
DROP POLICY IF EXISTS work_log_upd ON work_log;
CREATE POLICY work_log_ins ON work_log FOR INSERT
  WITH CHECK (fn_can_write('work_log')
              OR (fn_is_field() AND staff_id = fn_my_staff_id()));
CREATE POLICY work_log_upd ON work_log FOR UPDATE
  USING (fn_can_write('work_log')
         OR (fn_is_field() AND staff_id = fn_my_staff_id()))
  WITH CHECK (fn_can_write('work_log')
              OR (fn_is_field() AND staff_id = fn_my_staff_id()));

DROP POLICY IF EXISTS daily_report_write ON daily_report;
CREATE POLICY daily_report_write ON daily_report FOR ALL
  USING (fn_can_write('daily_report')
         OR (fn_is_field() AND staff_id = fn_my_staff_id()))
  WITH CHECK (fn_can_write('daily_report')
              OR (fn_is_field() AND staff_id = fn_my_staff_id()));

DROP POLICY IF EXISTS daily_issue_all ON daily_issue;
CREATE POLICY daily_issue_read ON daily_issue FOR SELECT
  USING (NOT fn_is_field()
         OR EXISTS(SELECT 1 FROM daily_report r
                    WHERE r.id = daily_issue.report_id AND r.staff_id = fn_my_staff_id()));
CREATE POLICY daily_issue_write ON daily_issue FOR ALL
  USING (fn_can_write('daily_issue')
         OR (fn_is_field() AND EXISTS(SELECT 1 FROM daily_report r
              WHERE r.id = daily_issue.report_id AND r.staff_id = fn_my_staff_id())))
  WITH CHECK (fn_can_write('daily_issue')
              OR (fn_is_field() AND EXISTS(SELECT 1 FROM daily_report r
                   WHERE r.id = daily_issue.report_id AND r.staff_id = fn_my_staff_id())));

-- ── ③ eng_setting：★按【键】判，不按表判 ─────────────────────────
--    这张表一表装六个部门的设置：财务 15 个键、运维 8 个、售前 7 个、库管 6 个、采购 5 个。
--    表级归属只写了 eng_mgmt，于是管理员改财务的 GST、运维的 SLA、库管的仓库地址，
--    统统不受授权约束 —— 按表判在这里是不够的。
--    ★决策级的键（安全阈值 / 发件身份 / 账号上限）先摘出来标成无部门归属，
--      否则管理员改自己该管的全局参数还要去求工程授权，那是另一种错。
--    （write_depts 本身有一道「只有决策管理员能改」的门禁 trg_setting_depts_guard，
--      DDL 跑的时候还没有任何账号，会被它拦下 —— 这里临时摘掉再挂回去）
ALTER TABLE eng_setting DISABLE TRIGGER setting_depts_guard;
UPDATE eng_setting SET write_depts = ARRAY[]::text[]
 WHERE key IN ('max_admin_accounts','otp_valid_minutes');
ALTER TABLE eng_setting ENABLE TRIGGER setting_depts_guard;
-- ⚠ 安全三阈值（sec_idle/stepup/stale）与发件身份四键（sender_*）**契约里至今没有** ——
--   §17 与四十九轮定的那些，原型里还是 JS 变量。等它们落库时，
--   记得一并登记成【无部门归属】（决策级），否则又会掉进「按表判归 eng_mgmt」的坑。

DROP POLICY IF EXISTS eng_setting_upd ON eng_setting;
CREATE POLICY eng_setting_upd ON eng_setting FOR UPDATE
  USING (
    -- 决策级的键（无部门归属）：管理员本职
    (coalesce(array_length(write_depts,1),0) = 0 AND fn_is_admin())
    -- 本部门的人：照旧（fn_in_dept 不含管理员万能通行）
    OR EXISTS(SELECT 1 FROM unnest(write_depts) AS d WHERE fn_in_dept(d))
    -- ★管理员改别的部门的键 ＝ 代部门操作，要那个部门授权
    OR (fn_is_admin() AND EXISTS(SELECT 1 FROM unnest(write_depts) AS d WHERE fn_dept_delegated(d)))
  )
  WITH CHECK (
    (coalesce(array_length(write_depts,1),0) = 0 AND fn_is_admin())
    OR EXISTS(SELECT 1 FROM unnest(write_depts) AS d WHERE fn_in_dept(d))
...
COMMENT ON TABLE eng_setting IS
  '全局设置，★按【键】控制写权限（一表装六个部门的设置）。'
  'v0.40：管理员改别的部门的键同样要那个部门授权 —— 按表判在这里不够，'
  '因为财务的 GST、运维的 SLA、库管的仓库地址都在这张表里';
...
('INV-DEL-04','★有写策略的表必须登记写入归属（登记空数组＝明确的决策级）','critical',
 $q$SELECT c.relname AS table_name
     ...
     WHERE n.nspname='public' AND c.relkind='r'
       ...
       AND NOT EXISTS (SELECT 1 FROM table_ownership o WHERE o.table_name = c.relname)$q$,
 '★没登记归属的表，对最高管理者是【敞开】的 —— 部门授权那道门管不到它，而且不报错。'
 '业务表登记它属于哪个部门；决策级的表登记成空数组（明确写下来，不靠"查不到就放行"）'),

('INV-DEL-05','★管理员的写权限只能有一个入口（不许绕过 fn_can_write）','high',
 $q$SELECT p.tablename, p.policyname, p.cmd
      FROM pg_policies p
     WHERE p.schemaname='public' AND p.cmd IN ('INSERT','UPDATE','ALL')
       AND (coalesce(p.qual,'')||coalesce(p.with_check,'')) LIKE '%fn_is_admin%'
       AND (coalesce(p.qual,'')||coalesce(p.with_check,'')) NOT LIKE '%fn_can_write%'
       AND (coalesce(p.qual,'')||coalesce(p.with_check,'')) NOT LIKE '%fn_admin_may_write%'
       AND (coalesce(p.qual,'')||coalesce(p.with_check,'')) NOT LIKE '%fn_dept_delegated%'
       -- ★明确登记成【空数组】的决策级表，本来就该由 fn_is_admin() 直接放行
       AND NOT EXISTS(SELECT 1 FROM table_ownership o
                       WHERE o.table_name = p.tablename
                         AND coalesce(array_length(o.write_dept,1),0) = 0)$q$,
 '★有部门归属的表，写策略里直接写 fn_is_admin() 就放行 = 绕过了部门授权那道门。'
 '代部门的写操作必须经 fn_can_write。决策级的表请在 table_ownership 里登记成空数组 ——'
 '★明确写下来，不要靠「查不到就当是本职」');
