...
CREATE TABLE project (
    ...
    -- 售前七步
    step1_intro_at    timestamptz,
    step2_design_at   timestamptz,
    step3_review_at   timestamptz,
    step4_draft_at    timestamptz,
    step5_revise_at   timestamptz,
    step6_signed_at   timestamptz,   -- 方案定版信号
    step7_deposit_at  timestamptz,   -- 财务写入(占位)
    ...
    quote_version     text,          -- 报价版本
    quote_status      text CHECK (quote_status IN ('draft','locked') OR quote_status IS NULL),
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    'payment_milestone','payment_receipt',
...
-- #####################################################################
-- ##  v0.27：业主订阅（归运维） + 留标/已烂尾 + 停留天数 + 利润率口径收紧
-- #####################################################################
...
BEGIN
    ...
    IF NEW.status = 'bid_lost' THEN
        ...
        IF settled_cnt > 0 THEN
            RAISE EXCEPTION '门禁：本项目已有 % 笔合同款结清，已进入收款阶段，不能置为「留标」。'
    ...
    IF NEW.status = 'stalled' THEN
        ...
        IF NEW.step6_signed_at IS NULL THEN
            RAISE EXCEPTION '门禁：本项目尚未签约进入施工，谈不上烂尾。未中标请用「留标」';
        END IF;
    END IF;
    RETURN NEW;
END;
...
SELECT p.id AS project_id, p.code, p.name, p.status,
       CASE p.status
         WHEN 'engaging' THEN '接洽当中' WHEN 'to_contact' THEN '待联系'
         ...
         WHEN 'bid_lost' THEN '留标' WHEN 'stalled' THEN '已烂尾' END AS status_cn,
       p.status_since,
...
SELECT p.code, p.name, p.status, st.status_cn,
       ...
       CASE WHEN p.status='bid_lost' THEN '留标'
...
('INV-SUB-04','订阅收款必须挂 kind=subscription','high',
 ...
 '否则会被算进合同款或维护款，污染利润率'),
('INV-STATUS-01','留标项目不得有已结清的合同款','critical',
...
SELECT c.id AS project_id, c.code, c.name, c.status,
       ...
       CASE
         ...
         ELSE 'breach' END AS margin_band,
       CASE
         WHEN c.status = 'bid_lost'  THEN '留标·无利润率'
...
INSERT INTO assertion_def(code,label,severity,query,hint) VALUES
('INV-MARGIN-02','留标/未开工项目不得显示利润率','high',
